 ...
 * h 是一个包装函数，主要是在 vnode 方法上再做一层包装
 *
 * 如：
...
export function h(sel: any, b?: any, c?: any): VNode {
  ...
  // 如果此方法有 3 个传参，则第二个参数认为是 data 数据
  if (c !== undefined) {
    data = b;
  ...
  } else if (b !== undefined) { // 如果传参 c 不存在，只存在传参 b，那么说明需要渲染的 vdom 不存在 data 部分，只存在 children 部分
  ...
  if (is.array(children)) {
    for (i = 0; i < children.length; ++i) {
      // 如果此节点是原始类型数据（string|number），说明该节点是 text 节点，就为此节点创建一个函数，如：
      // ' and this is just normal text' => vnode(undefined, undefined, undefined, children[i])
      // 即：将当前 children 中的 text 包装成 VNode 形式
